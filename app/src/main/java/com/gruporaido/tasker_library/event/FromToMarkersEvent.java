/**
 * GRUPO RAIDO CONFIDENTIAL
 * __________________
 *
 * [2015] - [2015] Grupo Raido Incorporated
 * All Rights Reserved.
 *
 * NOTICE: All information contained herein is, and remains
 * the property of Grupo Raido SAPI de CV and its suppliers,
 * if any. The intellectual and technical concepts contained
 * herein are proprietary to Grupo Raido SAPI de CV and its
 * suppliers and may be covered by México and Foreign Patents,
 * patents in process, and are protected by trade secret or
 * copyright law. Dissemination of this information or
 * reproduction of this material is strictly forbidden unless
 * prior written permission is obtained from Grupo Raido SAPI
 * de CV.
 */

package com.gruporaido.tasker_library.event;

public class FromToMarkersEvent {
    protected double mFromLatitude;
    protected double mFromLongitude;
    protected double mToLatitude;
    protected double mToLongitude;
    protected int mDriveTime;

    public FromToMarkersEvent(double fromLatitude, double fromLongitude, double toLatitude, double toLongitude, int driveTime) {
        mFromLatitude = fromLatitude;
        mFromLongitude = fromLongitude;
        mToLatitude = toLatitude;
        mToLongitude = toLongitude;
        mDriveTime = driveTime;
    }

    public double getFromLatitude() {
        return mFromLatitude;
    }

    public void setFromLatitude(double fromLatitude) {
        mFromLatitude = fromLatitude;
    }

    public double getFromLongitude() {
        return mFromLongitude;
    }

    public void setFromLongitude(double fromLongitude) {
        mFromLongitude = fromLongitude;
    }

    public double getToLatitude() {
        return mToLatitude;
    }

    public void setToLatitude(double toLatitude) {
        mToLatitude = toLatitude;
    }

    public double getToLongitude() {
        return mToLongitude;
    }

    public void setToLongitude(double toLongitude) {
        mToLongitude = toLongitude;
    }

    public int getDriveTime() {
        return mDriveTime;
    }

    public void setDriveTime(int driveTime) {
        mDriveTime = driveTime;
    }
}
