...
import com.gruporaido.tasker_library.fragment.FailedConnectionDialogFragment;
...
public class ErrorHandler {
    ...
    public static void handleError(APIResponseHandler handler, FragmentManager fragmentManager, int statusCode, JSONObject errorResponse) {
        if (statusCode >= 400 && statusCode < 500) {
            ...
            showDialogIfNoConnection(fragmentManager, statusCode, handler);
        } else {
            showDialogIfNoConnection(fragmentManager, statusCode, handler);
    ...
    public static boolean addErrorsToEditText(EditText field, JSONObject errors, String key) throws JSONException {
        if (errors.has(key)) {
    ...
    public static void showDialogIfNoConnection(FragmentManager fragmentManager, int statusCode, APIResponseHandler handler) {
        if (statusCode == 0) {
            FailedConnectionDialogFragment fragment = FailedConnectionDialogFragment.newInstance(handler);
